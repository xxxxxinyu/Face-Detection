...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        According to the lowest error, choose the classifier. For each feature, if 
        featureVals < 0, then set h,the classifier, equal 1. Then, use the formula
        to calculate error. When a new error is less than bestError, replace bestError
        with it, and record the classifier into bestClf.
        """
        error = 0
        bestError = 100000
        h = featureVals
        ...
        for i in range(rc[0]):
            for j in range(rc[1]):
              error += weights[j] * abs(h[i][j] - labels[j])
            print(error)
            if error < bestError:
              num = i
              bestError = error
              bestClf = WeakClassifier(features[num])
            error = 0
